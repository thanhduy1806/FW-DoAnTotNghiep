...
/* Maximum scratch buffer size for endian conversion.
 * Must be >= largest possible parameter size (double = 8 bytes).
 * Increase if DATA type params are larger. */
#define PARAM_SCRATCH_SIZE  256
...
static gs_error_t gs_param_serialize_array(gs_param_table_instance_t * tinst,
                                           ...
                                           unsigned int * buf_pos)
{
    ...
    /* Check total size fits in buffer */
    {
        unsigned int size = param_size * param_array_size;
        if ((flags & F_PACKED) == 0) {
            size += param_array_size * sizeof(uint16_t);
        }
    ...
    uint8_t value[PARAM_SCRATCH_SIZE];
    if (param_size > PARAM_SCRATCH_SIZE) {
        return GS_ERROR_OVERFLOW;
    }
...
gs_error_t gs_param_serialize_item_direct(gs_param_type_t param_type,
                                          ...
                                          unsigned int * buf_pos)
{
    ...
    if ((flags & F_PACKED) == 0) {
        uint16_t out_addr = (flags & F_TO_BIG_ENDIAN) ? util_htobe16(addr) : addr;
        if ((flags & F_DRY_RUN) == 0) {
            memcpy(&buf[*buf_pos], &out_addr, sizeof(out_addr));
        }
        *buf_pos += sizeof(out_addr);
    }

    if ((flags & F_DRY_RUN) == 0) {
        if ((flags & F_TO_BIG_ENDIAN) && gs_param_require_endian_swap(param_type)) {
            /* Static scratch buffer - no alloca */
            uint8_t tmp[PARAM_SCRATCH_SIZE];
            if (param_size > PARAM_SCRATCH_SIZE) {
                return GS_ERROR_OVERFLOW;
            }
            memcpy(tmp, item, param_size);
            gs_param_htobe(param_type, tmp);
            memcpy(&buf[*buf_pos], tmp, param_size);
        } else {
...
gs_error_t gs_param_serialize_item(const gs_param_table_row_t * param,
                                   uint16_t addr,
                                   const void * item,
                                   uint32_t flags,
                                   uint8_t * buf,
                                   ...
                                   unsigned int * buf_pos)
{
    return gs_param_serialize_item_direct(GS_PARAM_TYPE(param), GS_PARAM_SIZE(param),
                                          addr, item, flags, buf, buf_size, buf_pos);
}

gs_error_t gs_param_serialize_full_table(gs_param_table_instance_t * tinst,
                                         ...
                                         unsigned int * buf_pos)
{
    ...
    for (; i < tinst->row_count; i++) {
        error = gs_param_serialize_array(tinst, &tinst->rows[i], flags, buf, buf_size, buf_pos);
        if (error) {
            break;
        }
    }
                                     ...
                                     uint32_t flags)
{
    ...
    if ((flags & F_FROM_BIG_ENDIAN) && gs_param_require_endian_swap(param_type)) {
        /* Static scratch buffer - no alloca */
        uint8_t tmp[PARAM_SCRATCH_SIZE];
        if (param_size > PARAM_SCRATCH_SIZE) {
            return GS_ERROR_OVERFLOW;
    ...
    return gs_param_set(tinst, addr, param_type, item, param_size, GS_PARAM_FLAGS(param));
}

gs_error_t gs_param_deserialize(gs_param_table_instance_t * tinst,
                                const uint8_t * buf,
                                unsigned int buf_size,
                                uint32_t flags)
{
