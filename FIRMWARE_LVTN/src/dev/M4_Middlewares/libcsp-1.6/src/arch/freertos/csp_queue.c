...
typedef struct {
    StaticQueue_t  xStaticQueue;   /* first field */
    uint8_t       *pucStorage;     /* item storage buffer */
} csp_queue_ctx_t;

csp_queue_handle_t csp_queue_create(int length, size_t item_size) {

    csp_queue_ctx_t *ctx = pvCSPMalloc(sizeof(csp_queue_ctx_t));
    if (ctx == NULL) return NULL;

    ctx->pucStorage = pvCSPMalloc((size_t)length * item_size);
    if (ctx->pucStorage == NULL) {
        vCSPFree(ctx);
        return NULL;
    }
...
void csp_queue_remove(csp_queue_handle_t queue) {
    ...
    vQueueDelete(queue);
    csp_queue_ctx_t *ctx = (csp_queue_ctx_t *)queue;
    vCSPFree(ctx->pucStorage);
    vCSPFree(ctx);
}
